...
/**
 * create notification and queue serial data while the activity is not in the foreground
 * use a listener chain: SerialSocket -> SerialService -> UI fragment
 */
public class SerialService extends Service implements SerialListener {
    ...
    public void attach(SerialListener listener) {
        if(Looper.getMainLooper().getThread() != Thread.currentThread())
            throw new IllegalArgumentException("not in the main thread");
        cancelNotification();
        // use synchronized() to prevent new items in queue2
        // new items will not be added to queue1 because mainLooper.post and attach() run in the main thread
        synchronized (this) {
    ...
    public void detach() {
        if(connected)
            createNotification();
        // items already in the event queue (posted before detach() to mainLooper) will end up in queue1
